...
export function applyDaeEffects({ specList = [], completedSpecs = {}, allowAllSpecs = false, wildCardsInclude = [], wildCardsExclude = [], doStatusEffects = true }) {
    ...
    for (let c of changes) {
        ...
        if (c.value.includes("dae.eval(") || c.value.includes("dae.roll(")) {
            ...
            foundry.utils.mergeObject(conditionData, { stackCount, effect: c.effect.toObject() }); //shouldn't be needed separately /*, effectId: c.effect.id, effectUuid: c.effect.uuid,origin: c.effect.origin*/
      ...
      @ts-expect-error
      else if (field instanceof foundry.data.fields.BooleanField) fieldType === "boolean";
      ...
      if (fieldType !== "number" || c.mode === CONST.ACTIVE_EFFECT_MODES.CUSTOM)
...
export async function doActivityEffects(activity, activate, targets = undefined, activityEffectsUuids, options = {
    ...
    //@ts-expect-error
    const activityEffects = activityEffectsUuids.map(aeUuid => fromUuidSync(aeUuid)).filter(ef => ef).map(ef => ef.toObject());
...
export async function applyActivityEffects(activity, activate, targets, activityEffects, options) {
    ...
    if (gmTargets.length > 0) {
        await socketlibSocket.executeAsGM("applyActiveEffects", { userId: game.user?.id, activate, activeEffects: activityEffects, targetList: gmTargets, itemDuration: activity.item.system.duration, itemCardId: options.itemCardId, removeMatchLabel: options.removeMatchLabel, toggleEffect: options.toggleEffect, metaData: options.metaData });
    }
    if (localTargets.length > 0) {
        const result = await applyActiveEffects({ activate, targetList: localTargets, activeEffects: activityEffects, itemDuration: activity.item.system.duration, itemCardId: options.itemCardId, removeMatchLabel: options.removeMatchLabel, toggleEffect: options.toggleEffect, metaData: options.metaData, origin: options.origin });
    }
}
...
export function daeInitActions() {
    ...
    async function _preCreateActiveEffect(wrapped, data, context, user) {
        ...
        if (result)
            result = wrapped(data, context, user);
        return result;
    }
    ...
    async function _preUpdateActiveEffect(wrapped, ...args) {
        ...
        if (dependentConditions) {
            ...
            if ((this.active && statusesChanged) || changed.disabled !== undefined || changed.isSuppressed !== undefined) {
                if (debugEnabled > 0)
...
async function applyStatusEffectChagnes(actor, doc, currentStatuses, newStatuses, includeEffect) {
    ...
    for (let status of newStatuses.difference(currentStatuses)) {
        ...
        if (!statusEffect || (statusEffect._id === doc._id && includeEffect))
            continue;
...
// TODO once v12 only get rid of this since it is in core.
...
export function enumerateBaseValues(objectDataModels) {
    const baseValues = {};
    ...
    function processMappingField(key, mappingField, baseValues) {
        const fields = mappingField.initialKeys;
        if (!fields)
            return;
        for (let fieldKey of Object.keys(fields)) {
            if (mappingField.model instanceof SchemaField) {
                processSchemaField(`${key}.${fieldKey}`, mappingField.model, baseValues);
            }
            else if (mappingField.model instanceof MappingField) {
                processMappingField(`${key}.${fieldKey}`, mappingField.model, baseValues);
            }
            else {
    ...
    function processSchemaField(key, schemaField, baseValues) {
